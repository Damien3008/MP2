...
import pandas as pd
A_star_folder= "/home/damien/Desktop/School/Concordia/COMP_472/Mini_projects/MP2/A_star_tests"
...
for files in files_all:
    for file in files:
        if files == files_all[0]:
            f = open("/home/damien/Desktop/School/Concordia/COMP_472/Mini_projects/MP2/A_star_tests/" + file, 'r')
...
data = pd.DataFrame(cont, columns=["puzzle number", "algorithm", "heuristic", "length of the solution", "length of the path", "Execution time (seconds)"])
info = list()
for algo in ['ucs', 'gbfs', 'A*']:
    buf = list()
    if algo == 'ucs':
        for value in ["length of the solution", "length of the path", "Execution time (seconds)"]:
...
columns = ['length of the solution (for gbfs and A* heuristic = 1)','length of the path (for gbfs and A* heuristic = 1)',
           'Execution time (seconds) (for gbfs and A* heuristic = 1)', 'length of the solution (for gbfs and A* heuristic = 2)','length of the path (for gbfs and A* heuristic = 2)',
           'Execution time (seconds) (for gbfs and A* heuristic = 2)', 'length of the solution (for gbfs and A* heuristic = 3)','length of the path (for gbfs and A* heuristic = 3)',
           'Execution time (seconds) (for gbfs and A* heuristic = 3)', 'length of the solution (for gbfs and A* heuristic = 4)','length of the path (for gbfs and A* heuristic = 4)',
           'Execution time (seconds) (for gbfs and A* heuristic = 4)']
