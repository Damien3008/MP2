...
class GameEngine:
    ...
    def print_results(self, method, game, start, end, finishing_state, state_counter, ambulance, heuristic, index_a, solution, cars):
        file = open(method + '-' + 'sol' + '-' + game.split(" ")[1] + '.txt', 'w')
        ...
        while move_a > 0:
            ...
            move_a = move_a - 1
        file.write("Initial Board Configuration: " + str(current) + "\n\n")
        for item in current.state:
            file.write(str(item).replace(", ", " ").replace("[", " ").replace("]", " ").replace("'", ""))
            file.write('\n')
        file.write("\n")
        file.write("Car fuel available: " + fuel_levels + "\n")
        file.write("Runtime: " + str((end - start).total_seconds()) + " seconds\n")
        file.write("Search path length: " + str(state_counter) + " states\n")
        file.write("Solution path length: " + str(solution_counter) + " moves\n")
        file.write("Cost path solution: " + str(solution_counter) + '\n')
        file.write("Solution path: " + solution_path + '\n\n')        
        file.write(solution)
        file.write("\n")
    ...
    def extract_fuel(self, input):
        num = int(input.split(' ')[1])
        return Rush_Hour_Search.read_file(self)[-1][num - 1]
...
class State:
    def __init__(self, state, parent, offset, fuel_offset, heuristic=(1,1,1), g = 0, last_move=("Z", 0, 0)):
...
class Rush_Hour_Search(GameEngine):
    ...
    def A_star(self, heuristic = (1,1,1)):
        start = datetime.now()
        fuel = self.extract_fuel(self.game)
        solution = True
        open_list = []
        ...
        while current_state.h > 0:
            ...
            if len(open_list) > 0:
                ...
                solution = False
                break
        self.print_results('Astar', self.game, start, datetime.now(), current_state, len(closed_list), horizontal_cars[index_a], heuristic, index_a,
                    solution, horizontal_cars + vertical_cars)
